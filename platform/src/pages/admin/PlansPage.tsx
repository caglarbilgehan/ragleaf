...
export default function PlansPage() {
  ...
  const fetchPlans = async () => {
    ...
    try {
      ...
      toast.error(t('admin.plans.load_error'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(t('admin.plans.toast_updated'));
      setPlans(prev => prev.map(p => (p.id === updated.id ? updated : p)));
      handleCloseModal();
    } catch (error) {
      console.error('Error updating plan:', error);
      toast.error(t('admin.plans.toast_update_error'));
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="bg-dark-800/60 rounded-xl border border-white/[0.06] p-16 text-center">
          ...
          <h3 className="text-lg font-medium text-gray-200">{t('admin.plans.no_plans')}</h3>
          <p className="text-sm text-gray-500 mt-1">
            {t('admin.plans.no_plans_desc')}
          </p>
        </div>
      ) : (
                ...
                <div className="pt-2">
                  <div className="flex items-baseline text-gray-100">
                    ...
                    <span className="ml-1 text-sm font-semibold text-gray-500">
                      / {plan.billing_cycle === 'monthly' ? t('admin.plans.billing_monthly') : plan.billing_cycle}
                    </span>
                  </div>
                </div>
                ...
                <div className="border-t border-white/[0.06] pt-4 space-y-3">
                  <div className="flex items-center justify-between text-sm">
                    <div className="flex items-center gap-2 text-gray-400">
                      ...
                      <span>{t('admin.plans.max_agents')}</span>
                    </div>
                    <span className="font-semibold text-gray-200">
                      {plan.max_agents === -1 ? t('admin.plans.unlimited') : plan.max_agents}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center justify-between text-sm">
                    <div className="flex items-center gap-2 text-gray-400">
                      ...
                      <span>{t('admin.plans.max_docs')}</span>
                    </div>
                    <span className="font-semibold text-gray-200">
                      {plan.max_documents === -1 ? t('admin.plans.unlimited') : plan.max_documents}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center justify-between text-sm">
                    <div className="flex items-center gap-2 text-gray-400">
                      ...
                      <span>{t('admin.plans.query_limit')}</span>
                    </div>
                    <span className="font-semibold text-gray-200">
                      {plan.max_queries_per_month === -1 ? t('admin.plans.unlimited') : plan.max_queries_per_month.toLocaleString('tr-TR')}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center justify-between text-sm">
                    <div className="flex items-center gap-2 text-gray-400">
                      ...
                      <span>{t('admin.plans.max_storage')}</span>
                    </div>
                    <span className="font-semibold text-gray-200">
                      {plan.max_storage_mb === -1 ? t('admin.plans.unlimited') : `${plan.max_storage_mb.toLocaleString('tr-TR')} MB`}
                    </span>
                  </div>
                </div>
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
          <div className="relative w-full max-w-lg bg-dark-800 border border-white/[0.08] rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="flex items-center justify-between p-6 border-b border-white/[0.06]">
              <div>
                <h3 className="text-lg font-bold text-gray-100">{t('admin.plans.modal_title')}</h3>
            ...
            <form onSubmit={handleFormSubmit} className="flex-1 overflow-y-auto p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="col-span-2">
                  <label className="block text-xs font-medium text-gray-400 mb-1.5">{t('admin.plans.label_name')}</label>
                  <input
                    type="text"
                    required
                    value={formName}
                ...
                {/* Price */}
                <div>
                  <label className="block text-xs font-medium text-gray-400 mb-1.5">{t('admin.plans.label_price')}</label>
                  <input
                    type="number"
                    min="0"
                    required
                ...
                {/* Billing Cycle */}
                <div>
                  <label className="block text-xs font-medium text-gray-400 mb-1.5">{t('admin.plans.label_billing')}</label>
                  <select
                    value={formBillingCycle}
                    onChange={(e) => setFormBillingCycle(e.target.value)}
                    className="w-full px-3 py-2 border border-white/[0.1] bg-dark-700/50 text-gray-100 rounded-lg text-sm focus:ring-2 focus:ring-primary-500 focus:outline-none"
                  >
                    <option value="monthly">{t('admin.plans.billing_option_monthly')}</option>
                    <option value="yearly">{t('admin.plans.billing_option_yearly')}</option>
                  </select>
                </div>

                {/* Max Agents */}
                <div>
                  <label className="block text-xs font-medium text-gray-400 mb-1.5">{t('admin.plans.label_max_agents_hint')}</label>
                  <input
                    type="number"
                    min="-1"
                    required
                ...
                {/* Max Documents */}
                <div>
                  <label className="block text-xs font-medium text-gray-400 mb-1.5">{t('admin.plans.label_max_docs_hint')}</label>
                  <input
                    type="number"
                    min="-1"
                    required
                ...
                {/* Max Queries */}
                <div>
                  <label className="block text-xs font-medium text-gray-400 mb-1.5">{t('admin.plans.label_query_limit_hint')}</label>
                  <input
                    type="number"
                    min="-1"
                    required
                ...
                {/* Max Storage */}
                <div>
                  <label className="block text-xs font-medium text-gray-400 mb-1.5">{t('admin.plans.label_max_storage_hint')}</label>
                  <input
                    type="number"
                    min="-1"
                    required
                ...
                <div className="col-span-2 pt-2">
                  <label className="flex items-center gap-3 cursor-pointer">
                    ...
                    />
                    <span className="text-sm font-medium text-gray-300">{t('admin.plans.label_active_hint')}</span>
                  </label>
                </div>
              </div>
